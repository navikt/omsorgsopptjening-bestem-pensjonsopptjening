...
-- DROP TABLE OMSORGSVEDTAK_PERIODE;
-- DROP TABLE OMSORGS_SAK;
-- DROP TABLE OMSORGS_GRUNNLAG;
-- DROP TABLE FNR;
-- DROP TABLE PERSON;
...
-- DROP SEQUENCE omsorgsvedtak_periode_seq;
-- DROP SEQUENCE omsorgs_sak_seq;
-- DROP SEQUENCE omsorgs_grunnlag_seq;
...
CREATE SEQUENCE omsorgs_grunnlag_seq INCREMENT 50;
CREATE TABLE OMSORGS_GRUNNLAG
(
    OMSORGS_GRUNNLAG_ID BIGINT primary key not null,
...
CREATE SEQUENCE omsorgs_sak_seq INCREMENT 50;
CREATE TABLE OMSORGS_SAK
(
    OMSORGS_SAK_ID      BIGINT primary key not null,
    OMSORGS_GRUNNLAG_ID BIGINT             not null,
    CONSTRAINT fk_snapshot_sak FOREIGN KEY (OMSORGS_GRUNNLAG_ID) REFERENCES OMSORGS_GRUNNLAG (OMSORGS_GRUNNLAG_ID)
);



CREATE SEQUENCE omsorgsvedtak_periode_seq INCREMENT 50;
CREATE TABLE OMSORGSVEDTAK_PERIODE
(
    OMSORGSVEDTAK_PERIODE_ID BIGINT primary key not null,
    OMSORGS_SAK_ID     BIGINT             not null,
    FOM                      DATE               not null,
    TOM                      DATE               not null,
    PROSENT                  SMALLINT           not null,
    LANDSTILKNYTNING         VARCHAR(30)        not null,
    CONSTRAINT fk_sak_periode FOREIGN KEY (OMSORGS_SAK_ID) REFERENCES OMSORGS_SAK (OMSORGS_SAK_ID),
...
(
    OMSORGSVEDTAK_PERIODE_ID BIGINT not null,
    PERSON_ID                BIGINT not null,
    PRIMARY KEY (OMSORGSVEDTAK_PERIODE_ID, PERSON_ID),
    CONSTRAINT fk_periode_omsorgsarbeidsmottaker FOREIGN KEY (OMSORGSVEDTAK_PERIODE_ID) REFERENCES OMSORGSVEDTAK_PERIODE (OMSORGSVEDTAK_PERIODE_ID),
